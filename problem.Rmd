...
world_converge <- merge.data.frame(growth_world, pwtworld_1960, by = "country" , all = F)
...
``` {r}
library(ggplot2)
p_world <- ggplot(world_converge, aes(x = world_converge[,3], y = world_converge[,2], label = country)) + geom_point() + geom_text(aes(label=country))
```
...
``` {r}
p_world + geom_smooth(method = lm) + labs(x = "gdp per worker in 1960 in USD", y = "average growth 1960-2014 in %", title = "Lack of Convergence in the World")
```
